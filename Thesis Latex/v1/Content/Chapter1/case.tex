\section{Multi-Periodic \acp{SANN} in Esterel}
\label{sec:esterel-mapping}

The mono-periodic implementation of \ac{SANN} using the methodology in Listing~\ref{lst:ai-bro}, while being time predictable, has limitations.
...
It can be noted that by Proposition~\ref{lemma1}, any \ac{ANN} implemented using the approach in Figure~\ref{fig:tca-bb-n} is also an instance of \ac{SANN} defined using Definition~\ref{def:sann}.
%\pr{This is same as the lemma in section 2. Do we need this?}

\begin{theorem}
	\label{thm:bb-soundness}
	(Soundness) A mono-periodic \ac{SANN} given an input vector $\mathbf{i}$
...
%It is important to note that a multi-tick execution of an \ac{SANN} will not speed up the generation of outputs of the overall network --- as now, multiple ticks must be completed before a given output will be ready. 
...
\begin{table}[h]
	\centering
	\caption{Summary of \ac{SANN} approaches}
...
%\subsection{Implementing `Black-Box' \ac{SANN} in Esterel}
...
%This can be considered the `bare minimum' to qualify as a \ac{SANN}, as the network is now running synchronously with its environment.
