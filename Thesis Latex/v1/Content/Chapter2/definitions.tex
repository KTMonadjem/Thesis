...
\begin{definition}[Layer of a \ac{SCNN}]
...
\subsubsection{Running a \ac{SCNN}}
...
\ignore{Care has to be taken when running the \ac{SNN} like this to ensure that outputs reach the appropriate input of the next component in a synchronous manner so that deadlines are not missed and each component runs on the correct set of inputs, i.e. a \ac{SCNN} component should not run until all its inputs are present.}
