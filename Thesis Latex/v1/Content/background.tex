...
\subsection{Static Verification of \acfp{ANN}}
...
However, since \textit{A} and \textit{B} are signals, the \textbf{?} is used to fetch the value at that signal, rather than the presence (or absence) of the signal.
In Esterel a \textit{function} returns some value, this value can be stored in a variable using the operator \textbf{:=}, or it can be passed straight to the next function or, as with this example, be emitted by passing is as the parameter to the \textit{emit} statement.
...
Wait for A and B. 
When both A and B have been present, run the function \textit{runANN} with the integer values of A and B as its parameters.
Pause execution until the next tick.
Emit O with the returned value of \textit{getOutput} as its integer value.
Repeat.
