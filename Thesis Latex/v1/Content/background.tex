...
Esterel can, additionally, use \textit{pre()} statements to 
...
Usually, a \textit{loop} denotes an iterative thread that repeats each clock tick and each loop must be bounded by a \textit{pause}. 
However, execution of this loop only iterates when \textit{R} is detected, and the loop resets at the beginning of the next clock tick.
When \textit{R} is not detected, the loop does not loop, but instead pauses until \textit{R} is detected.
The \textit{loop ... each R} can be alternatively written as \textit{loop [abort [... pause;] when R] end loop}, i.e. take some action, pause at the end of the action, and then wait for \textit{R} to repeat next tick.

The \textit{||} are used to show parallel actions, i.e. concurrency, in the code.
This means that \textit{await A} and \textit{await B} are running concurrently, where \textit{await A} is synonymous with ``pause until A is present''.
...
Wait for A and B. 
When both A and B have been present, emit O.
When/if R is present, start from the beginning.
